...
options=("systeminfo" "hardwareinfo" "pciinfo" "securecopy" "logininfo" "uptime" "cronjobs" "pwd" "service restart" "ping" "uncompress(bz2)" "uncompress(gz)" "compress(tar)" "nmap" "Quit")
hardwareinfo=lshw
pcinfo=lspci
userlogedin=pinky
uptime=uptime
	   ...
	   ;;
        ...
        "compress(tar)")
         ...
         tar -cvf $filename $path 
           ;;
 	"nmap")
	echo Enter host\'s name to be scanned:
	read hostname
	echo Enter file name: 
	read filename
	nmap -v -T4 -A -sV -F $hostname >$filename
           ;;		
        "Quit")
           break
           ;;
